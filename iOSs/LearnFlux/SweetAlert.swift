...
public class SweetAlert: UIViewController {
    ...
    var userActionThree:((selectedButton: Int) -> Void)? = nil
    let kFont = "Helvetica"
    ...
    private func resizeAndRelayout() {
        ...
        if buttons.count == 3 {
            totalWidth = buttonRect[0].size.width + buttonRect[1].size.width + buttonRect[2].size.width + kWidthMargin + 40.0 * 2
        }
    ...
    func closeAlert(buttonIndex:Int){
        if (buttons.count == 2) {
            ...
            if userActionThree != nil {
                SweetAlertContext.shouldNotAnimate = true
                userActionThree!(selectedButton: buttonIndex)
                SweetAlertContext.shouldNotAnimate = false
            }
        }
    ...
    public func showAlert(title: String,
                          ...
                          thirdButtonColor: UIColor?,
                          action: ((selectedButton: Int) -> Void)? = nil) {
        userActionThree = action
        let window: UIWindow = UIApplication.sharedApplication().keyWindow!
        window.addSubview(view)
        window.bringSubviewToFront(view)
        view.frame = window.bounds
        ...
        if secondButtonTitle != nil && secondButtonTitle!.isEmpty == false {
            let button: UIButton = UIButton(type: UIButtonType.Custom)
            button.setTitle(secondButtonTitle, forState: UIControlState.Normal)
            button.backgroundColor = secondButtonColor
            button.addTarget(self, action: #selector(SweetAlert.pressed(_:)), forControlEvents: UIControlEvents.TouchUpInside)
            button.tag = 1
            buttons.append(button)
        }
        
        if thirdButtonTitle != nil && thirdButtonTitle!.isEmpty == false {
            let button: UIButton = UIButton(type: UIButtonType.Custom)
            button.setTitle(thirdButtonTitle, forState: UIControlState.Normal)
            button.backgroundColor = thirdButtonColor
            button.addTarget(self, action: #selector(SweetAlert.pressed(_:)), forControlEvents: UIControlEvents.TouchUpInside)
            button.tag = 2
            buttons.append(button)
        }
